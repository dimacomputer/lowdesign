...
/** Feature toggles for icon system */
if (!function_exists('ld_icons_features')) {
  function ld_icons_features(): array {
    ...
    if (!function_exists('ld_opt')) return $cache = $defaults;

    return $cache = [
      'content' => (bool) ld_opt('enable_content_icons', $defaults['content']),
      'terms'   => (bool) ld_opt('enable_term_icons',   $defaults['terms']),
      'menu'    => (bool) ld_opt('enable_menu_icons',   $defaults['menu']),
    ];
  }
}
...
/** Conditionally include ACF field groups */
add_filter('acf/load_field_group/key=group_menu_icon', function($g){
  return ld_icons_features()['menu'] ? $g : false;
});
add_filter('acf/load_field_group/key=group_term_icon', function($g){
  return ld_icons_features()['terms'] ? $g : false;
});
add_filter('acf/load_field_group/key=group_post_icon', function($g){
  return ld_icons_features()['content'] ? $g : false;
});

/** Hook ACF selects with full sprite ids (and mark wrappers for JS) */
if (ld_icons_features()['menu']) {
...
if (ld_icons_features()['content']) {
  ...
  if (!function_exists('ld_content_icon')) {
    ...
    function ld_content_icon($post_id = null, array $attrs = []): string {
      if (!ld_icons_features()['content']) return '';
...
if (!function_exists('ld_term_icon_html')) {
  ...
  function ld_term_icon_html($term = null, string $class = '', array $attrs = []): string {
    if (!ld_icons_features()['terms']) return '';
    if (!function_exists('get_field')) return '';
...
if (ld_icons_features()['content'] || ld_icons_features()['terms']) {
